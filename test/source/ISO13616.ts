...
test('ISO 13616/validate', (t) => {
	each`
		input                                | valid
		-------------------------------------|-------
		AD 00 82732793347266899771           | no
		AD 97 82732793347266899771           | yes
		IQ 01 NBIQ 850 123456789012          | no
		IQ 98 NBIQ 850 123456789012          | yes
		CA 02 0131 3167 8000 0065 1238 8     | yes
		CA 99 0131 3167 8000 0065 1238 8     | no
	...
	`((record) => {
		...
		t.equal(ISO13616.validate(input), valid === 'yes', `${input} is valid: ${valid}`);
	});

	t.end();
});

test('ISO 13616/checksum', (t) => {
	each`
		country | account                        | checksum
		--------|--------------------------------|----------
		CA      | 0131 3167 8000 0065 1238 8     | 02
	...
	`((record) => {
		const { country, account, checksum } = record as any;

		t.equal(ISO13616.checksum(account, country), checksum, `country ${country} and account ${account} creates checksum ${checksum}`);
	});

	t.end();
});

test('ISO 13616/generate', (t) => {
	each`
	...
	`((record) => {
		const { country, account, full, formatted } = record as any;

		t.equal(ISO13616.generate(account, country), full, `account ${account} and country ${country} generates formatting undefined generates ${full}`);
		t.equal(ISO13616.generate(account, country, false), full, `account ${account} and country ${country} generates formatting false generates ${full}`);
		t.equal(ISO13616.generate(account, country, true), formatted, `account ${account} and country ${country} generates formatting true generates ${formatted}`);
	});
...
test('ISO 13616/match (Symbol.match)', (t) => {
	...
	`((record) => {
		const { input, output } = record as any;

		t.equal(ISO13616.format(input), output, `formats "${input}" as "${output}"`);
	});
